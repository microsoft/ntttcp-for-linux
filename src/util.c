...
int write_result_into_json_file(struct ntttcp_test_endpoint *tep)
{
	...
	for(i = 0; i < tep->total_threads; i++ ){
		...
		else if (tep->results->threads[i]->is_sync_thread != true && i != 0) 
			fprintf(json_file, "  },\n");
		fprintf(json_file, "  {\n   \"index\":\"%i\",\n", i);
		...
		fprintf(json_file, "    {\n    \"metric\":\"KB/s\",\n");
       		fprintf(json_file, "    \"text\":\"%.3f\"\n    },\n", tepr->threads[i]->KBps);
		fprintf(json_file, "    {\n     \"metric\":\"MB/s\",\n");
		fprintf(json_file, "     \"text\":\"%.3f bps\"\n    },\n", tepr->threads[i]->MBps);
		fprintf(json_file, "    {\n     \"metric\":\"mbps\",\n");
		fprintf(json_file, "     \"text\":\"%.3f\"\n    }\n   ],\n",tepr->threads[i]->mbps);
		fprintf(json_file, "     \"avg_bytes_per_compl\":{\n     \"metric\":\"B\",\n");
		fprintf(json_file, "     \"text\":\"%.3f\"\n     }\n",0.000);
		if (i == tep->total_threads - 1)
			fprintf(json_file, "    }\n");
	}
	fprintf(json_file, "  ],\n");
	...
	fprintf(json_file, "    \"avg_bytes_per_compl\":{\n        \"metric\":\"B\",\n");
	fprintf(json_file, "	\"text\":\"%.3f\"\n    },\n", 0.000);
	fprintf(json_file, "    \"threads_avg_bytes_per_compl\":{\n        \"metric\":\"B\",\n");
	fprintf(json_file, "	\"text\":\"%.3f\"\n    },\n",0.000);
	fprintf(json_file, "    \"avg_frame_size\":{\n        \"metric\":\"B\",\n");
	fprintf(json_file, "	\"text\":\"%.3f\"\n    },\n",0.000);
	fprintf(json_file, "    \"throughput\":[\n");
	fprintf(json_file, "	{\n        \"metric\":\"MB/s\",\n");
	fprintf(json_file, "	\"text\":\"%.3f\"\n	},\n", tepr->throughput_MBps);
	...
	fprintf(json_file, "    \"avg_packets_per_interrupt\":{\n        \"metric\":\"packets/interrupt\",\n");
	fprintf(json_file, "	\"text\":\"%.3f\"\n    },\n", tepr->packets_per_interrupt);
	fprintf(json_file, "    \"interrupt\":{\n        \"metric\":\"count/sec\",\n");
	fprintf(json_file, "	\"text\":\"%.3f\"\n    },\n", 0.000);
	fprintf(json_file, "    \"dpcs\":{\n        \"metric\":\"count/sec\",\n");
	fprintf(json_file, "	\"text\":\"%.3f\"\n    },\n", 0.000);
	fprintf(json_file, "    \"avg_packets_per_dpc\":{\n        \"metric\":\"packets/dpc\",\n");
	fprintf(json_file, "	\"text\":\"%.3f\"\n    },\n", 0.000);
